...
| REST readiness | wireに`ready` boolean keyはなく、同じ完全受理rootの実在2 fieldについて`state == "ready" && authenticated == true`だけがreadyである。`health=200`、listener/process生存、auth child exitは別事実 | `docs/REST_API_V1.md` status schema/readiness、`UX-20260822-SSH-001` | `ready=true` keyの発明、health/process/exitだけでready化、`ready,false`または`auth_required,true`の受理 |
...
| 残量 | 有限な0..100%。欠測を利用量から推定せずlast-goodまたは未取得。期間左端から現在/確定終端までを使う | `DESIGN.md` Graph意味論、`GRAPH_PARITY_FIXTURE_CONTRACT_2026-08-22.md` | 終端だけ82%へ補間、アイドル中の変化、100%超への復帰 |
