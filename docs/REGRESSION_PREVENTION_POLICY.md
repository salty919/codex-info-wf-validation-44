...
7. 製品変更PRのmerge前にだけ`bash scripts/final_acceptance_gate.sh <Windows UI E2E evidence>`を1回通過させる。同ゲートは同一PR実行で既に完了したnative release build/CLI/recorder、main実適用merge rule、Windows installer/UI Automationを再実行せず、先行jobの成功とsource SHA/tree、status/detailsの同一世代受理マーカー、quota gauge証拠、過去期間グラフ、名前付き画面キャプチャ、物理window move証拠の対応と完全性を検証する。各画像のSHA-256は同じ実行のcapture行と一致しなければならない。非製品のみPRでは同ゲートとartifact処理をskipし、required acceptanceの最初のstepが分類値、version未変更、全producerの`skipped`を確認して成功する。分類欠落、期待外のjob結果、製品変更の証拠欠落・古いSHA/tree・SKIP・test 0件を`PASS`へ変換しない。required acceptance自体はpath filterやconditionで消さず、Release jobは製品変更だけ検証済みtreeとmerge結果の同一性を確認し、品質test/gateを再実行しない。

8. `main`向けPRの変更pathが`AGENTS.md`、`README.md`、`docs/**`、`.github/ISSUE_TEMPLATE/**`だけなら非製品のみと分類し、version変更、native/Windows/UIの製品build・test、artifact生成を0件にする。merge後もquality run解決、artifact download、binary build、manifest、tag、GitHub Releaseを0件にする。rename/copyは変更前後pathを判定し、mixed・未知path・workflow/CI script変更は製品変更の完全経路へ送る。identity、changed-files件数、全page、重複、空、schemaを完全検証できない場合は非製品扱いへfallbackせず失敗する。workflow全体をpath filterで消さず、required `version-prepared`と`acceptance`は常に生成する。製品変更ではversion準備済みheadだけが各artifact ownerを1回実行し、非製品のみでは全ownerが`skipped`であることをacceptanceが検査する。
