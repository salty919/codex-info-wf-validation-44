...
The exact details contract revision is `rest-v1-details-gap-20260823`; `history_gaps` contains only
...
| --- | --- | --- | --- | --- |
| WIN-INSTALL-01 | The Windows client must be delivered with an installer program. | The installer is a runnable Windows executable and contains the published client payload; a shortcut-only helper is not sufficient. | Installer build and payload manifest gate | verified |
| WIN-INSTALL-02 | Installation must add the client in the normal Windows way. | Per-user installation requires no administrator privilege, copies the complete published payload, creates a Start-menu entry, and registers an uninstall entry in the per-user Apps/Uninstall registry. Partial copy failure must not publish the Start-menu entry. | Isolated install fixture and Windows installer inspection | verified |
...
| WIN-INSTALL-04 | Uninstallation must remove the installed functionality. | It removes the Start-menu/Desktop shortcuts, uninstall registration, installed binaries, and installer metadata. It must not delete Codex history or client settings unless an explicit purge option is supplied. | Isolated uninstall fixture and registry/filesystem inspection | verified |
...
| WIN-PAR-06 | Preserve graph and history. | Current/old periods, dollar/token metric, model visibility, no-history state, persistence. | SQLite history and graph samples | Graph fixture tests, SQLite persistence tests, fresh current Graph image, independent parity review | implementation + current image; independent review pending |
...
| WIN-PAR-13 | Preserve persistence semantics. | History is minute-bucketed, same period/minute keeps the maximum, rows older than three months are pruned on normal startup, and reload does not fabricate values. | Native SQLite store and details endpoint | UsageStore tests, DB protection E2E, restart/reload hash evidence | verified |
