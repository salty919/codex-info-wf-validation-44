...
- workflowとCIの各拒否条件は、その条件だけが防ぐ具体的な不正状態とauthority入力を設計時に示す。既存条件から論理的に導ける重複条件、表示・移動用のURLや文言、判定に使わない値をgateにせず削除する。回帰testの期待値は実装から複製せず、外部の実観測値または固定契約をoracleにする。
