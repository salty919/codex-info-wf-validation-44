...
| REG-10 | Destructive maintenance and future migrations must be recoverable. | Project backup/migration tests are proven once by the `scripts/regression_guard.sh` all-target output; `scripts/db_protection_e2e.sh` is a separate bounded SQLite fixture covering three online-backup generations with `quick_check`/reload/row+file SHA-256 and restore-failure source preservation. | verified |
