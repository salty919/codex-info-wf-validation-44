...
| REG-05 | Every future change touching account, thread, local usage, history, graph, or timer paths must run the native regression gate before completion. | `cargo fmt --check`, `cargo check`, `cargo test`, `cargo build --release`, fresh `./run.sh` state traces/screenshots, and independent review. | verified |
