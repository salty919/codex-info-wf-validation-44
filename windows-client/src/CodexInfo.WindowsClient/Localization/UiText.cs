...
public static class LocalizationService
{
    private static readonly UiText Japanese = new(
        ...
        "SSHユーザー名とLinuxホスト名/IPまたはSSH configのHost aliasを入力し、「SSH転送を開始」を押してください。Linux側ではCodex InfoをUIなしのAPIモードで起動します。記録daemonも自動起動し、UIを閉じても履歴を保護します。", "ssh -N -L 8787:127.0.0.1:8787 user@linux-host", "CODEX_INFO_API_LISTEN=127.0.0.1:8787 ./run.sh", "コピー", "コピーしました", "続行", "設定を開く", "保存", "キャンセル", "閉じる", "最小化", "最大化", "言語", "外観", "接続済み", "接続中", "正常", "Linux 側の取得エラー", "接続エラー", "Linux 側で準備中", "Linux 側で認証が必要です", "残量不足", "残量警告", "リセット警告", "未取得", "認証を開始", "認証を確認", "再試行", "最新", "詳細データは未取得");
