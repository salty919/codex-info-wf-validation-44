...
boundary and must not hide or duplicate authoritative values.
...
- Existing native state ownership, reset-period semantics, token accounting,
  and failure isolation remain authoritative.
...
## SSH-001 / RC-061〜063 connection authority
...
- Setup is `server/API prepare → listener → health → status → auth-start when needed → separate auth-check → ready`.
  Health, status, auth-start, and auth-check are not interchangeable, and Setup/app confirmation is not repeated on
...
- Graph surface: one instance, current/older reset-period selection, dollar or
  ...
  current labels, unused intervals, and persisted SQLite history.
...
DPI authority is the OS-reported integer `dpi` from a
...
The existing `/v1/status` wire shape remains unchanged for compatibility.
`GET /v1/details` is versioned as `api_version: "v1"` and is read-only. Its
...
| WIN-UPDATE-03 | An untrusted or incomplete release must never be launched. | Accept only published non-prerelease exact-repository `windows-vX.Y.Z`; validate exact manifest schema, stable version/tag, installer name/URL authority, positive size and lowercase SHA-256; limit redirects and bytes; remove partial files. Any transport/response/integrity/launch failure keeps the installed version runnable and exposes no raw body/exception. | Core boundary fixtures, coordinator filesystem/launcher tests, tampered installer E2E | local independent PASS: updater Core 25/25 and full Core 120/120; same-SHA CI pending |
| WIN-QUAL-01 | Unit-testable Windows product logic must maintain at least 90% line coverage. | The measured boundary includes Core, ViewModels, Settings, Localization, Graphing, preview fixtures, and pure window geometry. Generated XAML, bootstrap/composition, window lifecycle, and Avalonia/ScottPlot drawing adapters are covered by physical Windows E2E instead; any state transition, persistence, parsing, graph projection, or formatting logic found in an adapter remains in the unit-coverage boundary until extracted. | Pinned `CodeCoverage.runsettings`, fail-closed contract gate, Cobertura line rate, physical Windows E2E | HOLD / PRODUCT_PENDING: this WSL environment has no dotnet/Windows coverage run; a previous local number is not current release evidence |
...
| WIN-PAR-03 | Preserve quota summary and period semantics. | Weekly/monthly, zero/full, warning/reset-boundary, null/error snapshots. | Status API | Protocol/presentation fixtures and six final host state images | verified |
...
| WIN-PAR-13 | Preserve persistence and acquisition semantics. | History is minute-bucketed, same period/minute keeps the maximum, rows older than three calendar months are pruned on normal startup, each DB/API/UI acquisition is at most one calendar month (44,640 minute buckets), and reload does not fabricate values. | Native SQLite store and details endpoint | Three-month retention boundary tests, one-month indexed-range/capacity tests, DB protection E2E, restart/reload hash evidence | verified |
| WIN-DES-01 | New request: Windows-native polished design. | Fluent visual hierarchy, icon affordances, keyboard/focus states, exact logical client matrix (fixed surfaces 900×480; Graph 940×640 initial/700×480 minimum/unbounded max), high-DPI layout, no clipped text or accidental empty space; no feature or state is removed. | Windows geometry/DPI/non-scroll authority, common theme and all windows | Fresh normal/minimum/high-DPI/topology images plus OS rect/DPI measurement and Start-menu/keyboard focus smoke | unverified |
...
| REG-03 | Valid local usage snapshots create minute history samples and model/token graph lines; one oversized or malformed tool record must not turn the whole period into idle. | Oversized-record collector test, recoverable rollout test, persisted SQLite before/after counts, and fresh graph screenshot. | implemented; native tests pass; fresh X/Windows graph evidence pending |
...
| REG-05 | Every future change touching account, thread, local usage, history, graph, or timer paths must run the native regression gate before completion. | `scripts/regression_guard.sh` performs working-tree/index/commit diff checks, `cargo fmt --check`, `cargo check --locked --all-targets`, `cargo test --locked --all-targets` with a non-zero executed count, exact mandatory history/graph tests including an unobserved-gap no-diagonal assertion, rolling-reset drift, periodic refresh last-good retention, and dedicated idle-band color/opacity assertions, `cargo build --release --locked`, fresh `./run.sh` state traces/screenshots, and independent review. Release additionally requires Windows E2E evidence whose log contains `graph-past-model-data: PASS` (LUNA/TERRA/SOL pixels detected inside the past-period plot), `graph-past-idle-band: PASS` (dedicated idle-band pixels detected in the same plot), and all 19 named non-empty PNG captures with valid signatures, so a period-selection/image-hash-only check or arbitrary copied images cannot pass. | implemented; local native regression guard PASS; Windows E2E and independent review pending |
...
| REG-08 | Multiple clients/servers must not duplicate, regress, or mix history across profile/account partitions. | SQLite unique `(partition_id,reset_at,timestamp)` key, transactional max/coalesce upsert, bounded busy timeout, cross-profile/account isolation, and concurrent-writer store test. | HOLD / PRODUCT_PENDING |
...
| REG-10 | Destructive maintenance and future migrations must be recoverable. | `scripts/db_protection_e2e.sh` runs a real `UsageStore` fixture through three online-backup generations with `quick_check`/reload/row+file SHA-256, injects backup-rotation and invalid-candidate failures, verifies source preservation, performs a checked online restore, and reopens the migrated DB. | verified |
